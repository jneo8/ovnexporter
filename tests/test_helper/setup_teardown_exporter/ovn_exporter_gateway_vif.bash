setup_file() {
    ...
    export GATEWAY_VIF_NS_NAME="upgrade_ns0" 
    export GATEWAY_VIF_NAME="upgrade_vif0"
    ...
    for container in $CHASSIS_CONTAINERS; do
        ...
        netns_add "$container" "$GATEWAY_VIF_NS_NAME"
        microovn_add_vif "$container" \
            "$GATEWAY_VIF_NS_NAME" "$GATEWAY_VIF_NAME"
        ping_start "$container" 10.42.${ctn_n}.1 "$GATEWAY_VIF_NS_NAME"
    done
}
...
teardown_file() {
    ...
    if [ -n "$GATEWAY_VIF_NS_NAME" ] && [ -n "$GATEWAY_VIF_NAME" ]; then
        local container
        for container in $CHASSIS_CONTAINERS; do
            microovn_delete_vif "$container" \
                "$GATEWAY_VIF_NS_NAME" "$GATEWAY_VIF_NAME"
            netns_delete "$container" "$GATEWAY_VIF_NS_NAME"
